package com.vismaad.naad.sharedprefrences;

public class SehajBaniPreferences {

    public static String Atree_PREFERENCES = "SehajBaniPreferences";
    public static String JWT_TOKEN = "jwt";
    public static String JWT_TOKEN_VALUE = "";

    public static String LOGIN_ID = "id";
    public static String LOGIN_ID_VALUE = "";

    public static String RAGGI_ID = "shabad";
    public static String RAGGI_ID_VALUE = "";


    public static String SKIP_BUTTON = "btnSkip";
    public static String SKIP_BUTTON_VALUE = "";



    public static String YES_FEEDBACK = "yes_feedback";
    public static String YES_FEEDBACK_VALUE = "";



    public static String YES_RATING = "yes_rating";
    public static String YES_RATING_VALUE = "";


   // public static String YES_GIVEN = "yes_given";
  //  public static String YES_GIVEN_VALUE = "";


    public static String ADS_SHABADS = "ads";
    public static int ADS_SHABADS_VALUE = 0;

    public static String Count = "count";
    public static int Count_VALUE = 0;

}
